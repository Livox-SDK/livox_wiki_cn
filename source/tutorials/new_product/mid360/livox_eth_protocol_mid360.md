...
| 20260921         | v1.4.12  | 1. 新增360L相关协议描述                                        |

# 协议概述

此文档作为livox Mid 360系列雷达产品通信协议，含Mid 360、Mid 360S、Mid 360L
...
| 点云数据 | 雷达  --->上位机 | 56300    | 可配置(默认56301) | 默认单播(支持组播，360L除外) | UDP      |
| IMU数据  | 雷达  --->上位机 | 56400    | 可配置(默认56401) | 默认单播(支持组播，360L除外) | UDP      |
...
| --------------- | --------------------- | -------- | ----------- | ------------------------------------------------------------ | -------------------------------------------------- |
| 0x0000          | pcl_data_type         | 1        | uint8_t     | 雷达点云格式，详见[数据类型](2.3 数据类型)：<br/>0x01: 直角坐标(32bits)<br/>0x02: 直角坐标(16bits)<br/>0x03: 球坐标(360L不支持) | 是                                                 |
...
| 0x0021          | speed_mode            | 1        | uint8_t     | 0: 电机正常模式<br/>1: 电机低速模式(360S)<br/>2: 电机高速模式(360L)            | 是                                                 |
| 0x0026          | time_filter           | 1        | uint8_t     | 0: 时间戳正常同步，如果时间回退会导致点云中断<br/>1: GPS时间同步增加异常时间过滤且时间回退不会导致点云中断<br/>默认值为0  | 是                           |
| 0x0029          | pc_freq_mod(360L)           | 1        | uint8_t     | 点频模式：<br/>0: 点频80k<br/>1: 点频50k<br/>2: 点频100k                        | 是                                                 |
