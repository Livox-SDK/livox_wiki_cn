...
| ---------- | ------ | -------- |
| **日期**     | **版本** | **修改描述** |
| 20260730   | v1.0   | 1. 初始版本  |

# 协议概述

此文档仅作为livox Avia2雷达产品通信协议
...
![](./images/Avia2_state_machine.png)
...
| --------- | ------- | ------- | ---------------------------------------- |
| version   | 0       | 1       | 包协议版本：当前为1                               |
| length    | 1       | 2       | 从version开始的整个UDP数据段长度                    |
| reserved  | 3       | 2       | 保留                                       |
| dot_num   | 5       | 2       | 当前UDP包data字段包含点数目                        |
| udp_cnt   | 7       | 2       | 点云UDP包计数，每个UDP包依次加1，点云帧起始包清0<br/>        |
| frame_cnt | 9       | 1       | 此字段暂未启用                                  |
| data_type | 10      | 1       | 数据类型，详见(数据类型)                            |
| time_type | 11      | 1       | 时间戳类型，详见(时间戳)                            |
| reserved1 | 12      | 12      | 保留                                       |
| crc32     | 24      | 4       | timestamp+data段校验码，使用CRC-32算法(详见(CRC算法说明) ) |
| timestamp | 28      | 8       | 点云时间戳，详见(时间戳)                            |
| data      | 36      | --      | 数据信息，详见(数据类型)                            |

## 时间戳

每100ms内的点为一帧，此100ms内数据包使用相同时间戳
...
数据类型有以下3种，默认点云数据类型为1：

| 数据类型 | 采样类型  | 回波模式  | 坐标系模式 |
| ---- | ----- | ----- | ----- |
| 0    | IMU数据 |       |       |
| 1    | 点云数据  | 单回波模式 | 直角坐标系 |
| 17   | 点云数据  | 双回波模式 | 直角坐标系 |
...
| intensity | 12     | uint8_t | 信号强度       |
| 标签        | 13     | uint8_t | 当前暂未启用     |
...
| 字段         | 偏移(字节) | 数据类型    | 描述             |
| ---------- | ------ | ------- | -------------- |
| x_1        | 0      | int32_t | 回波1 X 轴，单位: mm |
| y_1        | 4      | int32_t | 回波1 Y 轴，单位: mm |
| z_1        | 8      | int32_t | 回波1 Z 轴，单位: mm |
| intensity1 | 12     | uint8_t | 信号强度1          |
| 标签1        | 13     | uint8_t | 当前暂未启用         |
| x_2        | 14     | int32_t | 回波2 X 轴，单位: mm |
| y_2        | 18     | int32_t | 回波2 Y 轴，单位: mm |
| z_2        | 22     | int32_t | 回波2 Z 轴，单位: mm |
| intensity2 | 26     | uint8_t | 信号强度2          |
| 标签2        | 27     | uint8_t | 当前暂未启用         |
...
| cmd_id      | 8      | 2       | 不同类型的消息通过此字段进行区分，详见(指令ID)                |
...
| crc_16      | 18     | 2       | 包头校验码，校验内容为从`sof`开始到`crc_16(不包括)`的共18字节数据,  使用CRC-16/CCITT-FALSE算法(详见(CRC算法说明) ) |
| crc32       | 20     | 4       | crc32校验码：data字段校验码,  使用CRC-32算法(详见(CRC算法说明) )<br/>当data字段长度为0时，CRC32校验码需填充为0. |
| data        | 24     | n       | 详见(指令详解)                                 |
...
| ---- | ------------- | ------ | ----------- | ---------------------------------------- |
| data | ret_code      | 0      | uint8_t     | 返回码:<br>(详见(返回码说明) )                     |
...
| --------- | --------------------- | ---- | ----------- | ---------------------------------------- | ------------------------------- |
| 0x0000    | pcl_data_type         | 1    | uint8_t     | 雷达点云格式，详见(数据类型)：<br/>0x01: 单回波直角坐标<br/>0x11: 双回波直角坐标 | 是                               |
...
| 0x001A    | work_tgt_mode         | 1    | uint8_t     | 雷达目标工作模式(即目标工作状态)，详见(雷达状态)               | 是                               |
| 0x001C    | imu_data_en           | 1    | uint8_t     | 0: IMU数据推送关闭<br/>1: IMU数据推动使能            | 是                               |
| 0x0022    | fov_mode              | 1    | uint8_t     | 0: Focus Detection Mode<br/>1: Normal Detection Mode, 350K points/s<br/>2: Normal Detection Mode, 300K points/s<br>3: Normal Detection Mode, 250K points/s | 是                               |
| 0x0024    | echo_mode             | 1    | uint8_t     | 0: 按照回波强度排序输出单回波输出最强回波<br/>1: 按照回波时间排序输出，单回波输出第一个返回回波 | 是                               |
| 0x0025    | NTP_server_ip         | 4    | uint8_t[4]  | NTP服务器的IP地址                              | 是                               |
| 0x0027    | ITO_ctrl              | 1    | uint8_t     | 窗口玻璃加热使能:<br>0: 关闭；<br>1: 强制开启加热<br>2: 恒温自动控制加热 | 是                               |
| 0x0028    | fog_noise_filter      | 1    | uint8_t     | 雨雾滤噪使能：<br>0: 关闭;<br>1: 雨滤除;<br>2: 雾滤除   | 是                               |
...
| 0x0811    | hms_code              | 32   | uint32_t[8] | 故障诊断码，每一条非0值代表一条诊断信息，当雷达工作异常时，可通过查询该诊断码确认问题原因； | 否                               |
...
|      | dev_type        | 6      | uint8_t  | 设备类型：<br>40：Avia2                        |
