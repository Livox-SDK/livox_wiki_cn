...
在没有PTP/gPTP, GPS的情况下，可以使用纯软件的NTP同步方式，该方式精度较低，但仅需一个NTP服务器即可。
...
NTP同步仅依赖一个NTP服务器，NTP 是“够用就好”的通用标准。精度级别为ms级。使用UDP端口123，适用于对时间同步精度要求较低的场景;

NTP和PTP都使用客户端/服务器（或主/从）模式，并通过交换带有时间戳的数据包来计算时间偏差，但它们处理延迟的方式截然不同:

1. NTP计算的关键假设是网络往返路径的延迟是对称的(即发送延迟T2-T1大致等于接收延迟T4-T3)。
2. 软件处理NTP数据包也会引入不确定的延迟。
3. PTP虽然也是软件协议，但它依赖硬件时间戳(Hardware Timestamping)。网卡(NIC)和交换机在数据包实际离开或到达物理接口的瞬间打上时间戳，而不是在操作系统软件层。


.. figure:: ../../../image/timesync/ntp.png